...
var generateMap = function(geoLat, geoLon) {
  ...
  function createListings(events) {
    ...
    for (var i=0; i<events.length; i++) {
      ...
      listings += '<div id="listing" class="listings" ' + dataHTML + '"><span class="titles">' + events[i].title + '</span><br><span>' + events[i].venue.name +'</span><span class="time">' + concertTime.toLocaleTimeString() + '</span></div>';
    }
